...
for(j=0;j<SIZE;j++)
{
//la cellule n'est pas attribuée
if(matrix[i][j] == 0)
{
*row = i;
*col = j;
//il y a une ou plusieurs cellules non attribuées
num_unassign = 1;
...
for(i=0;i<SIZE;i++)
{
//il y a une cellule avec la méme valeur
if(matrix[r][i] == n)
return 0;
}
...
//vérification de la sous-matrice
int row_start = (r/3)*3;
int col_start = (c/3)*3;
...
for(i=1;i<=SIZE;i++)
{
//si nous pouvons attribuer i à la cellule ou non
...
if(solve_sudoku())
return 1;
//si nous ne pouvons pas procéder avec cette solution
//réaffecter la cellule
matrix[row][col]=0;
}
}
return 0;
